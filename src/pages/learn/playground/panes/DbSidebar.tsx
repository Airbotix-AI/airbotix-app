// The Database window's LEFT panel (creative-code-studio-website-prd D-WEB-18):
// a real db-tool table list — every server-side table as a selectable row
// (icon · name · live row count), the selected one clearly marked, plus the
// `database.sqlite` identity line (file name + size from the same introspection
// response) pinned to the bottom. Pure presentation: selection state lives in
// DbPane (by NAME, so it survives the ~2s re-introspection polls).
