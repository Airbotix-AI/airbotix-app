...
// (live row counts, `db-table-<name>`, first table auto-selected) and a RIGHT
// panel showing the SELECTED table as the D-WEB-16 editable grid (`DbTable`:
// inline cell edit / add row / two-step delete, keyed by rowid), wrapped in
// `db-collection-<name>`. The toolbar carries Refresh + freshness + the
// selected table's `data/*.json` seed jump + the explicit two-step Reset
// database (the ONLY reset path — rebuilds every table from the seeds).
//
  ...
  return (
    <div data-testid="site-db-pane" className="flex h-full min-h-0 flex-col bg-pg-surface">
      ...
      {tables === null && pollError && projectId ? (
        ...
        // Master–detail (D-WEB-18): tables left, the selected grid right. The
        // sidebar keeps a sane min width; a narrow pane scrolls the grid.
