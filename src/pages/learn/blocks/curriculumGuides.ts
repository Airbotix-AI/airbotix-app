    ...
    storyPages: [
      {
        emoji: '🐒💥🌊',
        title: '路线到了，世界还没回应',
        body: '石猴保留 P4 的五块路线，脚底刚好碰到入口。现在要让两个 On Bump 事件都真实回应。',
        scene: 1,
      },
      {
        emoji: '🌊🫥',
        title: '水帘负责隐藏',
        body: '选 Water Curtain，把 Hide 放进 On Bump 与 Chime 之间。Hide 不能放到洞口角色上。',
        scene: 3,
      ...
      },
      {
        emoji: '🪨👁',
        title: '洞口负责出现',
        body: '再选 Cave Entrance，把 Show 放进它的 On Bump 与 Say 之间。按 Go，让真实碰撞同时启动两个轨。',
        scene: 4,
        blocks: ['💥 On Bump → 👁 Show', '💬 Say → 🏁 End'],
      },
    ],
    partnerLine: '水帘和洞口各有一个工作，角色归属必须正确。',
    mission: '在 Water Curtain 的碰撞轨放 Hide，在 Cave Entrance 的碰撞轨放 Show；保留石猴五块路线，按 Go 真实运行并保存。',
    question: '只 Hide 水帘、不 Show 洞口，会看见什么？',
    choices: [],
    retry: '空崖壁没有石桥、干地、石座和清水；洞口必须由自己的 Show 出现。',
    successTitle: '两个碰撞回应都运行了！',
    success: '水帘隐藏、洞口出现，Chime 与暖光一起证明真实碰撞发生。',
    fixTitle: '连接两个角色的空位',
    fixPrompt: '先选水帘放 Hide，再选洞口放 Show；不要改变石猴路线。',
    workspaceIntro: '这是两个真实事件轨，不是页面动画。',
    fixChoices: [],
    fixRetry: '',
    coach: {
      ready: '先选 Water Curtain，把 Hide 放入碰撞轨。',
      watch: '再选 Cave Entrance，把 Show 放入它的碰撞轨。',
      sayFirst: '先预测：只 Hide 会留下什么？',
      sayThen: '现在按 Go，比较两个轨的真实结果。',
      hopFirst: '石猴正沿 P4 的五块路线接近入口。',
      hopThen: '脚底碰到入口格，两个 On Bump 同时开始。',
      retry: '检查角色归属：水帘=Hide，洞口=Show。',
      fix: '两个空位都要由你亲手连接。',
      test: '按 Go，确认水帘消失、洞口出现。',
      saving: '碰撞结果正在保存……',
      complete: '两个事件轨都运行到了 End。',
    },
    logicSteps: [
      { icon: '🫥', label: '水帘 Hide', order: '碰撞后' },
      { icon: '👁', label: '洞口 Show', order: '同时回应' },
    ],
    logicWhy: '碰撞是原因，两个角色各自的事件轨共同形成可见结果。',
    completionTitle: 'Build 2 完成！',
    completion: '保存的作品含正确角色归属、真实碰撞与完整运行证据。',
    completionSteps: [
      { icon: '💥', label: '真实碰撞', order: '先' },
      { icon: '🌊', label: '水帘隐藏', order: '再' },
      { icon: '🪨', label: '洞口出现', order: '同时' },
    ],
    completionWhy: '只改页面状态不能通过；证据来自保存的 BlocksProject 和真实运行标记。',
    next: '洞里适合伙伴进入，下一步沿原路返回。',
  },
