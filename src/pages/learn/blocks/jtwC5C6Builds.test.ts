...
function one(lessonId: string, pageId: string, blocks: readonly Block[]): BlocksProject {
  return {
    version: 1,
    name: lessonId === 'jtw-s1-c6-p8' ? "My Monkey King's First Journey" : 'build',
    lessonId,
    pages: [
      {
        id: pageId,
...
describe('Journey West C5/C6 real Studio contracts', () => {
  it('offers preset dialogue for every required Say so free typing is never completion evidence', () => {
    expect(JTW_C5_C6_SAY_CHOICES).toEqual({
      'jtw-s1-c5-p5': ['Suitability is the goal'],
      'jtw-s1-c5-p7': ['Suitability is the goal'],
      'jtw-s1-c6-p4': ['Responsible for Tianma', 'Monkey King'],
      'jtw-s1-c6-p5': ['Tiangong responds'],
      'jtw-s1-c6-p8': [
  ...
  it.each([
    ['jtw-s1-c5-p4', 'jtw-c5-p4-page', C5_P4_TARGET],
    ['jtw-s1-c5-p5', 'jtw-c5-p5-page', C5_P5_TARGET],
    ['jtw-s1-c5-p7', 'jtw-c5-p7-page', C5_P7_TARGETS[0]],
  ] as const)('accepts %s only with its saved exact chain', (id, page, target) => {
  ...
  it('requires all three independently built C6 pages for the Personal Ship', () => {
    ...
    project.pages.push(
      one('x', 'jtw-c6-page-2', C6_PAGE_TWO).pages[0],
      one('x', 'jtw-c6-page-3', C6_PAGE_THREE).pages[0],
    );
    expect(jtwC5C6BuildMatches(project)).toBe(true);
