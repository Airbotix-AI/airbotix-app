...
async function runBothEvents() {
  ...
  fireEvent.click(within(screen.getByTestId('jtw-c4p2-prediction')).getByRole('button', { name: '名字出现，本领保持安静' }))
  fireEvent.click(screen.getByTestId('jtw-c4p2-go'))
  await waitFor(() => expect(screen.getByTestId('jtw-c4p2-flag-trace')).toHaveAttribute('data-trace', 'when_flag,show,say,hop,end'))
  ...
  await waitFor(() => expect(screen.getByTestId('jtw-c4p2-tap-trace')).toHaveAttribute('data-trace', 'when_tap,turn_right,end'))
}
...
describe('JourneyWestC4Part2Page', () => {
  ...
  it('requires the story and correct no-tap prediction before the real Go run', async () => {
    ...
    fireEvent.click(screen.getByTestId('jtw-c4p2-read'))
    fireEvent.click(within(screen.getByTestId('jtw-c4p2-prediction')).getByRole('button', { name: '他会自己先Hop，不必等Tap' }))
    expect(screen.getByTestId('jtw-c4p2-go')).toBeDisabled()
  ...
  it('measures separate real flag and tap traces, with reset between them', async () => {
    ...
    await screen.findByTestId('jtw-part-c4-p2')
    await runBothEvents()
    expect(screen.getByTestId('jtw-c4p2-early-hop')).toHaveTextContent('抢跑')
    expect(screen.getByTestId('jtw-c4p2-resolved')).toHaveTextContent('什么时候做')
