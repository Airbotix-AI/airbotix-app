...
beforeEach(() => {
  fetchProgress.mockResolvedValue({ story_line_id: 'journey-to-the-west-s1', unlocked_part_ids: ['jtw-s1-c4-p6'], completed: [{ part_id: 'jtw-s1-c4-p5', completed_at: '2026-08-08T00:00:00Z', evidence: { schema_version: 1, selections: { skill_version: ['leaf'] } } }] });
...
describe('JourneyWestC4Part6Page', () => {
  it('requires prediction, real buggy run, first deviation and repaired dual-run project', async () => {
    ...
    await waitFor(() => expect(completePart).toHaveBeenCalledWith('journey-to-the-west-s1', 'jtw-s1-c4-p6', expect.objectContaining({ selections: expect.objectContaining({ trigger_diff: ['when_flag->when_tap'], skill_version: ['leaf'] }) })));
  });
});
