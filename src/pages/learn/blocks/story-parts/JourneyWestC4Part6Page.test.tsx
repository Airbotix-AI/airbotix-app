...
beforeEach(() => {
    ...
    completed: [
      {
        ...
        evidence: { schema_version: 1, selections: { version: ['hop'] } },
      },
    ],
  });
  listProjects.mockResolvedValue([
    { id: 'p6-build', title: 'P6', kind: 'blocks', status: 'active' },
  ]);
  loadProject.mockResolvedValue({
    ...
    storyProgress: {
      schemaVersion: 1,
      completed: { 'jtw-s1-c4-p6': { completedAt: '2026-08-08T00:00:00Z' } },
    },
    project: {
      version: 1,
      name: 'P6',
...
describe('JourneyWestC4Part6Page', () => {
  it('requires prediction, real buggy run, first deviation and repaired dual-run project', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('jtw-c4p6-bug-trace')).toHaveTextContent('hop'), {
      timeout: 5000,
    });
    fireEvent.click(screen.getByRole('button', { name: 'The skill chain uses when_flag' }));
