...
beforeEach(() => {
  ...
  listProjects.mockResolvedValue([
    { id: 'project-c4-p5', title: 'C4 P5', kind: 'blocks', status: 'active' },
  ]);
  loadProject.mockResolvedValue({
    ...
    storyProgress: {
      schemaVersion: 1,
      completed: { 'jtw-s1-c4-p5': { completedAt: '2026-08-11T00:00:00.000Z' } },
    },
  });
...
describe('JourneyWestC4Part5Page', () => {
  it('requires motive, matching version, prediction, and saved Go/Tap evidence', async () => {
    ...
    fireEvent.click(
      screen.getByRole('button', {
        name: /Because the partner was ready first, Wukong responded to the invitation/i,
      }),
    );
      ...
      expect(completePart).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          selections: expect.objectContaining({
            version: ['hop'],
            run_trace: ['flag:name-only-end', 'partner-prediction', 'tap:hop:visible-end'],
          }),
          prediction: 'chosen-after-tap',
        }),
      ),
    );
  });
  ...
  it('rejects a selected version that does not match the saved program', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Turn around and point to home/i }));
    fireEvent.click(screen.getByRole('button', { name: /Go only displays the name/i }));
