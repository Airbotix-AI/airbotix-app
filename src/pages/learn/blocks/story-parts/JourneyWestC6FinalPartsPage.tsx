...
export function JourneyWestC6FinalPartsPage({ partId, previewSleep = async () => undefined }: { partId: PartId; previewSleep?: (ms: number) => Promise<void> }) {
  ...
  const [reasons, setReasons] = useState<string[]>([]); const [build, setBuild] = useState<string[]>([]); const [trace, setTrace] = useState<string[]>([])
  ...
  const [projectId, setProjectId] = useState<string | null>(null); const [version, setVersion] = useState<number | null>(null); const [snapshot, setSnapshot] = useState<string | null>(null); const [reopened, setReopened] = useState(false)
  ...
  const done = saved || (number === 3 && exactOrder && reasons.length === 2 && trace.length > 0) || (number === 4 && build.length >= 5 && reasons.length === 2 && trace.some((item) => item.includes('goto:2'))) || (number === 5 && build.length >= 5 && reasons.length === 2 && trace.some((item) => item.includes('goto:3'))) || (number === 6 && reasons.length >= 3 && trace.length > 0 && peer) || (number === 7 && bugTrace.some((item) => item.includes('planned:forever')) && fixed && trace.at(-1)?.includes('end') && trace.join('|') === repeatTrace.join('|')) || (number === 8 && projectId && version && reopened && trace.length > 0 && peer) || (number === 9 && retell.length === 4 && trace.length > 0 && peer) || (number === 10 && trace.at(-1)?.includes('end') && prediction === 'stable-mountain' && reasons.length === 2)

  const run = async () => setTrace(await runC6(c6Project(), previewSleep))
  const save = async () => { const project = c6Project(); const created = await createBlocksProject({ title: project.name, template: 'blocks_blank' }); const blank = await loadBlocksProject(created.id); const result = await saveBlocksProject({ projectId: created.id, project, version: blank.version, otherFiles: blank.otherFiles, history: { past: [], future: [] }, storyProgress: { schemaVersion: 1, completed: { 'jtw-s1-c6-p8': { completedAt: new Date().toISOString() } } } }); if (result.status === 'saved') { setProjectId(created.id); setVersion(result.version); setSnapshot(JSON.stringify(project)); setReopened(false) } }
  const reopen = async () => { if (!projectId || !snapshot) return; const loaded = await loadBlocksProject(projectId); setReopened(JSON.stringify(loaded.project) === snapshot) }
  const complete = useMutation({ mutationFn: () => completeStoryPart(JTW_S1_STORY_LINE_ID, partId, { schema_version: 1, prediction: prediction ?? undefined, selections: {
    ...
    build_ast: build, cause_links: reasons, run_trace: trace, before_ast: number === 5 ? PAGE_TWO_BUG.map((item) => item.op) : [], after_ast: number === 5 ? PAGE_TWO.map((item) => item.op) : [], peer_order: peer ? ['confirmed'] : [], personal_choices: reasons,
    ...
    build_project: projectId ? [projectId] : p8ProjectId ? [p8ProjectId] : [], saved_version: version ? [String(version)] : [], reopen_json_match: reopened ? ['true'] : [], peer_retell: peer ? ['same-version'] : [], three_page_trace: trace,
  ...
  return <div className="mx-auto max-w-3xl space-y-6 px-4 py-8" data-testid={`jtw-${partId}`}><header><p className="text-xs font-bold text-brand-sky">西游记 · 第六章 · Part {number}</p><h1 className="text-3xl font-black">{TITLES[partId]}</h1></header>
    ...
    {number === 4 && <Build title="搭 Page 1：两句、移动、停点、Page 出口" choices={PAGE_ONE.slice(1).map((item) => item.op)} build={build} setBuild={setBuild} reasons={reasons} setReasons={setReasons} onRun={run} />}
    {number === 5 && <Build title="重排 Page 2：行动、回应、时间影、Page 出口" choices={PAGE_TWO.slice(1).map((item) => item.op)} build={build} setBuild={setBuild} reasons={reasons} setReasons={setReasons} onRun={run} />}
    ...
    {number === 8 && <><p>三页作品至少 14 块、两个事件入口、两处节奏、两个 Page 出口和稳定 End；保存后必须关闭重开。</p><Choice option={{ id: 'three-page-plan', label: '我画好三页停点，并预测最终稳定 End', correct: true }} active={prediction === 'three-page-plan'} onPick={() => setPrediction('three-page-plan')} /><button className="btn-pill-primary" disabled={!prediction} onClick={() => void run()}>完整运行三页</button><button className="btn-pill-primary" disabled={!trace.length} onClick={() => void save()}>保存真实 VFS 版本</button>{projectId && <button className="btn-pill-secondary" onClick={() => void reopen()}>关闭并重开核对三页 JSON</button>}{reopened && <Choice option={{ id: 'peer', label: '伙伴从第一页预测后两页', correct: true }} active={peer} onPick={() => setPeer(true)} />}</>}
...
function Build({ title, choices, build, setBuild, reasons, setReasons, onRun }: { title: string; choices: string[]; build: string[]; setBuild: (value: string[]) => void; reasons: string[]; setReasons: (value: string[]) => void; onRun: () => Promise<void> }) { return <><p>{title}</p><Multi choices={choices} values={build} set={setBuild} /><Multi choices={['原因连接 1', '原因连接 2']} values={reasons} set={setReasons} /><button className="btn-pill-primary" disabled={build.length < 5 || reasons.length < 2} onClick={() => void onRun()}>运行真实三页程序</button></> }
