...
function answerPeerCorrectly() {
  pickPeer(1, /Turn to Page 2 · /i);
  pickPeer(2, /Turn to Page 3 · /i);
  pickPeer(3, /The story ends on this page/i);
}
...
describe('JourneyWestC3Part7Page — My three-page journey to seek a teacher', () => {
  ...
  it('reads the whole teaching text across two screens and names the work', async () => {
    ...
    expect(screen.getByTestId('jtw-c3p7-story-count')).toHaveTextContent(/1\s*\/\s*2\s*part/i);
    expect(screen.getByTestId('jtw-c3p7-story')).toHaveTextContent(/You will write the three pages/i);
    fireEvent.click(screen.getByTestId('jtw-c3p7-story-next'));
    expect(screen.getByTestId('jtw-c3p7-story-count')).toHaveTextContent(/2\s*\/\s*2\s*parts?/i);
    expect(screen.getByTestId('jtw-c3p7-story')).toHaveTextContent(/save it, close it, reopen it/i);
    // The motive lock is stated where the child reads it, not only in code.
    expect(screen.getByText(/He went to learn from a teacher/i)).toBeInTheDocument();
  });
  ...
  it('keeps the ship shut while the saved route is still the empty starter', async () => {
    ...
    await screen.findByTestId('jtw-part-c3-p7');
    await waitFor(() =>
      expect(screen.getByTestId('jtw-c3p7-build')).toHaveAttribute(
        'data-build-state',
        'in_progress',
      ),
    );
  ...
  it('refuses a reopen whose document is not the one that was saved', async () => {
    ...
    expect(screen.getByTestId('jtw-c3p7-reopen-result')).toHaveTextContent(/will not match the saved one/i);
    expect(screen.getByTestId('jtw-c3p7-run-button')).toBeDisabled();
    expect(screen.getByTestId('jtw-c3p7-continue')).toBeDisabled();
  });
  ...
  it('MEASURES the reopened route: 1 → 2 → 3, every boundary continuous', async () => {
    ...
    expect(screen.getByTestId('jtw-c3p7-seal-note')).toHaveTextContent(
  ...
  it('names the FIRST page the peer and the real run disagree on', async () => {
    ...
    pickPeer(1, /Turn to Page 1 · /i);
    pickPeer(2, /The story ends on this page/i);
    pickPeer(3, /The story ends on this page/i);
    fireEvent.click(screen.getByTestId('jtw-c3p7-reopen-button'));
    await waitFor(() =>
      expect(screen.getByTestId('jtw-c3p7-reopen')).toHaveAttribute('data-match', '1'),
    ...
    expect(screen.getByTestId('jtw-c3p7-mismatch')).toHaveTextContent(/Just fix this page/i);
    expect(screen.getByTestId('jtw-c3p7-continue')).toBeDisabled();
  ...
  it('persists the whole personal-ship evidence bundle and unlocks only C3-P8', async () => {
    ...
    expect(selections.peer_predictions).toEqual(['page1:page-2', 'page2:page-3', 'page3:ends']);
