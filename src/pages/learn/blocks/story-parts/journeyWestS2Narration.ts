...
const AUDIO_ROOT = '/story-blocks/journey-to-the-west/audio/en-AU/s2';
...
function narration(
  ...
  title: string,
  story: readonly string[],
): JourneyWestS2Narration {
  return {
    partId,
    title,
    story,
    text: [title, ...story].join('. '),
    audioPath: `${AUDIO_ROOT}/${partId}-v02.mp3`,
  };
}

export const JOURNEY_WEST_S2_NARRATIONS: readonly JourneyWestS2Narration[] = [
  narration(JTW_S2_C1_P1_ID, 'Turn a long journey into three steps today', S2_C1_P1_STORY),
  narration(JTW_S2_C1_P2_ID, 'Why does the note only have three lines?', S2_C1_P2_STORY),
