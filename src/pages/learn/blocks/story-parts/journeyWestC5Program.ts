...
export type C5EarlyPartId = Extract<
  C5PartId,
  'jtw-s1-c5-p1' | 'jtw-s1-c5-p2' | 'jtw-s1-c5-p3' | 'jtw-s1-c5-p4' | 'jtw-s1-c5-p5'
>;
...
export function c5BuildValid(ops: BlockOp[]): boolean {
  return (
    ops.length >= 4 &&
    ops.filter((op) => op === 'grow' || op === 'reset_size' || op === 'shrink').length === 3 &&
    ops.includes('wait') &&
    !ops.includes('turn_right')
  );
}
