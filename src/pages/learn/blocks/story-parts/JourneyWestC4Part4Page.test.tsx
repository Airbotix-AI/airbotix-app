...
beforeEach(() => {
  ...
  listProjects.mockResolvedValue([
    { id: 'project-c4-p4', title: 'C4 P4', kind: 'blocks', status: 'active' },
  ]);
  ...
  completePart.mockResolvedValue({
    part_id: 'jtw-s1-c4-p4',
    completed_at: '2026-07-28T02:00:00.000Z',
  });
});
  ...
  it('requires both predictions and the saved dual-run project before continuing', async () => {
    ...
    fireEvent.click(
      screen.getByRole('button', {
        name: /Just press Go: the name chain runs, the skill chain remains quiet/i,
      }),
    );
    fireEvent.click(
      screen.getByRole('button', { name: /The leaf pattern target will light up only after Tap/i }),
    );
    await waitFor(() => expect(screen.getByTestId('jtw-c4p4-continue')).toBeEnabled());
    expect(screen.getByTestId('jtw-c4p4-resolved')).toHaveTextContent(
      /first remembers the name.*only sees the small display/i,
    );
  });
