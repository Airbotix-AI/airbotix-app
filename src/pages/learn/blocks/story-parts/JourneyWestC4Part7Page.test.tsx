...
function renderPage() {
  ...
  render(
    <QueryClientProvider client={client}>
      <MemoryRouter initialEntries={['/learn/story/journey-west/jtw-s1-c4-p7']}>
        <Routes>
          <Route path="/learn/story/journey-west/:partId" element={<JourneyWestC4Part7Page />} />
...
beforeEach(() => {
  vi.mocked(storyApi.fetchStoryLineProgress).mockResolvedValue({
    ...
    unlocked_part_ids: ['jtw-s1-c4-p7'],
    completed: [],
  });
...
describe('JourneyWestC4Part7Page', () => {
  it('requires dual-event prediction, real saved run, peer discovery and exact reopen read-back', async () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'Go only gets its name, and Tap shows it' }),
    );
    fireEvent.click(screen.getByRole('button', { name: /Jump over the leaf pattern/i }));
    await waitFor(() =>
      expect(screen.getByTestId('jtw-c4p7-build-status')).toHaveTextContent('VFS'),
    );
    fireEvent.click(
      screen.getByRole('button', {
        name: 'Look at the name first, then find out Wukong can be clicked from the slight fingertip target',
      }),
    );
      ...
      expect(storyApi.completeStoryPart).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          selections: expect.objectContaining({
            saved_version: ['2'],
            reopen_json_match: ['true'],
            peer_discovery: ['found-wukong-by-gentle-cue'],
          }),
        }),
      ),
    );
  ...
  it('rejects a reopened project whose saved JSON changed', async () => {
    vi.mocked(blocksApi.loadBlocksProject)
    ...
    fireEvent.click(screen.getByTestId('jtw-c4p7-reopen'));
    await waitFor(() =>
      expect(screen.getByTestId('jtw-c4p7-reopen-status')).toHaveTextContent(
