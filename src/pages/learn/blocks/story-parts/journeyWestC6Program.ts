...
const block = (op: BlockOp, n?: number, text?: string): Block => ({
  op,
  ...(n ? { n } : {}),
  ...(text ? { text } : {}),
});
...
export const PAGE_ONE = [
  block('when_flag'),
  block('say', undefined, 'Responsible for Tianma'),
  block('wait', 5),
  block('set_speed', 2),
  block('move_right', 2),
  block('say', undefined, 'Monkey King'),
  block('goto_page', 2),
];
export const PAGE_TWO_BUG = [
  block('when_flag'),
  block('set_speed', 3),
  block('show'),
  block('say', undefined, 'The storm escalates'),
  block('show'),
  block('goto_page', 3),
];
export const PAGE_TWO = [
  ...
  block('show'),
  block('wait', 5),
  block('say', undefined, 'Tiangong responds'),
  block('set_speed', 1),
  block('wait', 5),
  block('goto_page', 3),
];
export const PAGE_THREE_BUG = [
  block('when_flag'),
  block('say', undefined, 'Stop here on the first trip'),
  block('wait', 5),
  block('stop'),
  block('forever'),
];
    ...
    characters: [
      {
        ...
        name: 'Wukong',
        emoji: '🐒',
        start: { gx: 5, gy: 9, size: 2, rot: 0, visible: true },
        scripts: [{ id: `${id}-story`, blocks }],
      },
    ],
  };
}

export function c6Project(
  pageOne = PAGE_ONE,
  pageTwo = PAGE_TWO,
  ...
  return {
    version: 1,
    name: "My Monkey King's First Journey",
    lessonId: 'jtw-s1-c6-p8',
    pages: [
      page('page-1', pageOne, 'sky-palace'),
      page('page-2', pageTwo, 'palace-shadows'),
      page('page-3', pageThree, 'five-elements-mountain'),
    ],
  };
}

export async function runC6(
  ...
  for (const current of project.pages) {
    const runner = new BlocksRunner(
      ...
      {
        onSprite: () => undefined,
        onSay: (_id, text) => trace.push(`${current.id}:say:${text}`),
        onNote: () => undefined,
        onSound: () => undefined,
        onGotoPage: (number) => trace.push(`${current.id}:goto:${number}`),
        onStep: (_id, _script, index) => {
          if (index >= 0)
            trace.push(`${current.id}:${current.characters[0].scripts[0].blocks[index]?.op}`);
        },
      },
      sleep,
    );
