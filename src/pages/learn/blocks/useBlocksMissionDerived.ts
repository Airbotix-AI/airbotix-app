...
import { JTW_C4_P4_LESSON_ID } from './jtwC4DualBuild'
...
export function useBlocksMissionDerived({
  ...
  const selectedChar =
    page.characters.find((character) => character.id === charId) ??
    page.characters[0]
  const storyMission = useMemo(
    () => storyMissionFor(project.lessonId),
    [project.lessonId],
  )
  ...
  const missionScript = useMemo(
    () =>
      page.characters
        ...
        .find(
          (script) =>
            script.id === storyMissionScriptId(storyMission?.lessonId ?? ''),
        ),
  ...
  const duetFirst = page.characters.find(
    (character) => character.id === TINY_STAR_DUET_FIRST_ID,
  )
  const duetSecond = page.characters.find(
    (character) => character.id === TINY_STAR_DUET_SECOND_ID,
  )
  ...
  const isJtwC4DualBuild = lessonId === JTW_C4_P4_LESSON_ID
