...
describe('JtW C4-P4 dual-event build contract', () => {
  ...
  it.each(['hop', 'turn', 'reappear'] as const)(
    'accepts a complete C4-P7 %s Personal Ship',
    (version) => {
...
describe('JtW C4-P5 expression choice contract', () => {
  ...
  it('rejects a P4 carry-over and a deleted name chain', () => {
    ...
    built.pages[0].characters[0].scripts[1].blocks = [
      { op: 'when_tap' },
      { op: 'hop', n: 2 },
      { op: 'say', text: "I'm waiting for the invitation" },
      { op: 'end' },
  ...
  it('accepts every carried-forward action group only after its trigger becomes Tap', () => {
    for (const version of ['hop', 'turn', 'reappear'] as const) {
      ...
      const targets = {
        ...
        turn: [
          { op: 'when_tap' as const },
          { op: 'turn_left' as const, n: 2 },
          { op: 'wait' as const, n: 1 },
          { op: 'say' as const, text: 'Home is over there' },
          { op: 'end' as const },
        ],
        reappear: [
          { op: 'when_tap' as const },
          ...
          { op: 'wait' as const, n: 1 },
          { op: 'show' as const },
          { op: 'say' as const, text: 'Look here again' },
          { op: 'end' as const },
        ],
      };
