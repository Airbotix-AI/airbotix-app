...
function messageProject(lessonId: string): BlocksProject {
  ...
  return {
    version: 1, name: lessonId, lessonId,
    pages: [{
      id: `${lessonId}-page`,
      background: c4 ? 'jtw-s2-c4-gaolao-courtyard' : 'jtw-s2-c5-flowing-sands-river',
      characters: c4 ? [
        { id: 'wukong-sender', name: 'Wukong', emoji: '🐒', start: { gx: 3, gy: 7, size: 2, rot: 0 }, scripts: [{ id: 'wukong-route-send', blocks: JTW_S2_C4_P4_WUKONG_TARGET.map((block) => ({ ...block })) }] },
        { id: 'bajie-receiver', name: 'Bajie', emoji: '🐷', start: { gx: 10, gy: 8, size: 2, rot: 0 }, scripts: [{ id: 'bajie-route-receive', blocks: (lessonId === 'jtw-s2-c4-p4' ? JTW_S2_C4_P4_BAJIE_TARGET : JTW_S2_C4_P5_BAJIE_TARGET).map((block) => ({ ...block })) }] },
        { id: 'white-horse-waiting', name: 'Horse', emoji: '🐴', start: { gx: 15, gy: 8, size: 2, rot: 0, visible: false }, scripts: [{ id: 'horse-route-receipt', blocks: (lessonId === 'jtw-s2-c4-p4' ? ([{ op: 'when_message', n: 3 }, { op: 'end' }] as Block[]) : JTW_S2_C4_P5_HORSE_TARGET).map((block) => ({ ...block })) }] },
      ] : [
        { id: 'wukong-relay', name: 'Wukong', emoji: '🐒', start: { gx: 2, gy: 7, size: 2, rot: 0 }, scripts: [
          { id: 'wukong-relay-send', blocks: JTW_S2_C5_P4_WUKONG_TARGET.map((block) => ({ ...block })) },
          ...(lessonId !== 'jtw-s2-c5-p4' ? [{ id: 'wukong-relay-receipt', blocks: JTW_S2_C5_P5_WUKONG_REPLY.map((block) => ({ ...block })) }] : []),
        ] },
        { id: 'bajie-relay', name: 'Bajie', emoji: '🐷', start: { gx: 9, gy: 8, size: 2, rot: 0 }, scripts: [{ id: 'bajie-relay-forward', blocks: JTW_S2_C5_P4_BAJIE_TARGET.map((block) => ({ ...block })) }] },
        { id: 'wujing-relay', name: 'Wujing', emoji: '🧑', start: { gx: 16, gy: 8, size: 2, rot: 0, visible: false }, scripts: [{ id: 'wujing-relay-receive', blocks: (lessonId === 'jtw-s2-c5-p4' ? JTW_S2_C5_P4_WUJING_TARGET : JTW_S2_C5_P5_WUJING_TARGET).map((block) => ({ ...block })) }] },
      ],
    }],
  }
}
...
function c6Project(lessonId: string): BlocksProject {
  return {
    version: 1, name: lessonId, lessonId,
    pages: [
      { id: `${lessonId}-page-1`, background: 'jtw-s2-c6-team-gather', characters: [
        { id: 'xuanzang-team', name: 'Xuanzang', emoji: '🧑', start: { gx: 2, gy: 8, size: 2, rot: 0 }, scripts: [{ id: 'xuanzang-team-start', blocks: JTW_S2_C6_P1_TARGET.map((block) => ({ ...block })) }] },
      ] },
      { id: `${lessonId}-page-2`, background: 'jtw-s2-c6-team-bridge', characters: [
        { id: 'bajie-team', name: 'Bajie', emoji: '🐷', start: { gx: 3, gy: 8, size: 2, rot: 0 }, scripts: [{ id: 'bajie-team-bridge', blocks: JTW_S2_C6_P2_TARGET.map((block) => ({ ...block })) }] },
      ] },
      { id: `${lessonId}-page-3`, background: 'jtw-s2-c6-team-west', characters: [
        { id: 'wujing-team', name: 'Wujing', emoji: '🧑', start: { gx: 4, gy: 8, size: 2, rot: 0, visible: false }, scripts: [{ id: 'wujing-team-finish', blocks: JTW_S2_C6_WUJING_TARGET.map((block) => ({ ...block })) }] },
        { id: 'white-horse-team', name: 'Horse', emoji: '🐴', start: { gx: 8, gy: 8, size: 2, rot: 0 }, scripts: [{ id: 'horse-team-finish', blocks: JTW_S2_C6_HORSE_TARGET.map((block) => ({ ...block })) }] },
        { id: 'wukong-team', name: 'Wukong', emoji: '🐒', start: { gx: 13, gy: 8, size: 2, rot: 0 }, scripts: [{ id: 'wukong-team-finish', blocks: JTW_S2_C6_WUKONG_TARGET.map((block) => ({ ...block })) }] },
      ] },
    ],
  }
}

function c3Project(lessonId: string, distance = 4, stoneGx = 6, horse: Block[] = JTW_S2_C3_P5_HORSE_TARGET): BlocksProject {
  return {
    version: 1, name: lessonId, lessonId,
    pages: [{
...
function project(lessonId: string, blocks: Block[], args?: { second?: Block[]; name?: string }): BlocksProject {
  const chapter2 = lessonId.startsWith('jtw-s2-c2-')
  const pageId = `${lessonId}-page`
  const scriptId = lessonId === 'jtw-s2-c1-p4' ? 'xuanzang-departure' :
    lessonId === 'jtw-s2-c1-p5' ? 'xuanzang-departure' :
      lessonId === 'jtw-s2-c1-p6' ? 'xuanzang-departure-debug' :
        lessonId === 'jtw-s2-c1-p7' ? 'xuanzang-personal-departure' : 'xuanzang-approaches-mountain'
  return {
    version: 1,
    ...
    pages: [{
      ...
      characters: [
        ...
        ...(args?.second ? [{
          ...
          scripts: [{ id: lessonId === 'jtw-s2-c2-p5' ? 'wukong-answers' : lessonId === 'jtw-s2-c2-p6' ? 'wukong-answers-too-early' : chapter2 ? 'wukong-waits' : 'mountain-sign-tap', blocks: args.second.map((block) => ({ ...block })) }],
        }] : []),
      ],
    }],
  }
...
describe('Journey West S2 Studio mission contracts', () => {
  ...
  it('requires the P4 route and an isolated On Tap sign response in P5', () => {
    const valid = project('jtw-s2-c1-p5', JTW_S2_C1_P4_TARGET, { second: JTW_S2_C1_P5_SIGN_TARGET })
  ...
  it('rejects the shipped early-arrival bug and accepts moving Wait between the routes', () => {
    const bug = [
      { op: 'when_flag' }, { op: 'say', text: '行囊带好' }, { op: 'move_right', n: 3 },
      { op: 'move_right', n: 3 }, { op: 'wait', n: 2 }, { op: 'say', text: '到了' }, { op: 'end' },
    ] as Block[]
    expect(jtwS2BuildMatches(project('jtw-s2-c1-p6', bug), 'jtw-s2-c1-p6')).toBe(false)
    expect(jtwS2BuildMatches(project('jtw-s2-c1-p6', JTW_S2_C1_P6_TARGET), 'jtw-s2-c1-p6')).toBe(true)
  })

  it('accepts only the three bag lines and two pace choices for Personal Ship', () => {
    const personal: Block[] = [
      { op: 'when_flag' }, { op: 'say', text: '带书卷出发' }, { op: 'set_speed', n: 1 },
      { op: 'move_right', n: 3 }, { op: 'wait', n: 2 }, { op: 'move_right', n: 3 },
      { op: 'say', text: '到了山下' }, { op: 'end' },
    ]
    expect(jtwS2BuildMatches(project('jtw-s2-c1-p7', personal, { name: 'My First Westward Departure' }), 'jtw-s2-c1-p7')).toBe(true)
    personal[1] = { op: 'say', text: '带魔法传送门出发' }
    expect(jtwS2BuildMatches(project('jtw-s2-c1-p7', personal, { name: 'My First Westward Departure' }), 'jtw-s2-c1-p7')).toBe(false)
  })

  it('keeps Wukong quiet while Xuanzang runs the C2-P4 Start script', () => {
    const valid = project('jtw-s2-c2-p4', JTW_S2_C2_P4_TARGET, { second: [{ op: 'when_tap' }, { op: 'end' }] })
    expect(jtwS2BuildMatches(valid, 'jtw-s2-c2-p4')).toBe(true)
    valid.pages[0].characters[1].scripts[0].blocks.splice(1, 0, { op: 'say', text: '我在这里' })
  ...
  it('requires the preserved question before the exact hidden Wukong Tap response', () => {
    ...
    valid.pages[0].characters[0].scripts[0].blocks[3] = { op: 'say', text: '我替你回答' }
    expect(jtwS2BuildMatches(valid, 'jtw-s2-c2-p5')).toBe(false)
  })
  ...
  it('requires P6 wrong-run, fixed-Go, correct answer and Tap evidence together', () => {
    expect(isJtwS2C2P6TapCompletion('jtw-s2-c2-p6', 'wukong-waiting', true, true, true, true)).toBe(true)
    expect(isJtwS2C2P6TapCompletion('jtw-s2-c2-p6', 'wukong-waiting', false, true, true, true)).toBe(false)
    expect(isJtwS2C2P6TapCompletion('jtw-s2-c2-p6', 'wukong-waiting', true, false, true, true)).toBe(false)
    expect(isJtwS2C2P6TapCompletion('jtw-s2-c2-p6', 'wukong-waiting', true, true, false, true)).toBe(false)
    expect(isJtwS2C2P6TapCompletion('jtw-s2-c2-p6', 'xuanzang', true, true, true, true)).toBe(false)
  })
  ...
  it('accepts all three aligned C3-P7 distances and both welcome versions', () => {
    const versionB: Block[] = [
      { op: 'when_bump' }, { op: 'show' }, { op: 'grow', n: 1 }, { op: 'reset_size' },
      { op: 'say', text: '我会稳稳同行' }, { op: 'end' },
    ]
    for (const [distance, stone] of [[3, 5], [4, 6], [5, 7]] as const) {
      const versionA: Block[] = [{ op: 'when_bump' }, { op: 'show' }, { op: 'hop', n: 1 }, { op: 'say', text: '欢迎同行' }, { op: 'end' }]
      expect(jtwS2BuildMatches(c3Project('jtw-s2-c3-p7', distance, stone, versionA), 'jtw-s2-c3-p7')).toBe(true)
      expect(jtwS2BuildMatches(c3Project('jtw-s2-c3-p7', distance, stone, versionB), 'jtw-s2-c3-p7')).toBe(true)
    }
    expect(jtwS2BuildMatches(c3Project('jtw-s2-c3-p7', 3, 7), 'jtw-s2-c3-p7')).toBe(false)
  })

  it.each([[3, 5], [4, 6], [5, 7]] as const)(
    'runs the real C3-P7 bump at distance %i and reveals the horse across the stream',
    async (distance, stone) => {
      const says: string[] = []
      const welcome: Block[] = [{ op: 'when_bump' }, { op: 'show' }, { op: 'hop', n: 1 }, { op: 'say', text: '欢迎同行' }, { op: 'end' }]
      const runner = new BlocksRunner(c3Project('jtw-s2-c3-p7', distance, stone, welcome).pages[0], {
        onSprite: () => undefined,
        onSay: (_id, text) => { if (text) says.push(text) },
        onSound: () => undefined,
        onNote: () => undefined,
      ...
      expect(says).toContain('欢迎同行')
      expect(says.filter((line) => line === '欢迎同行')).toHaveLength(1)
    },
  )
