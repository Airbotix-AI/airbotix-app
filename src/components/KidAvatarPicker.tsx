  ...
  return (
    <fieldset>
      ...
      <p className="mb-4 text-[13px] text-slate2">
        This helps everyone recognise the right kids page.
      </p>
      ...
      {(Object.keys(GROUP_LABELS) as Array<keyof typeof GROUP_LABELS>).map((category) => (
        <div key={category} className="mb-4">
          <div className="mb-2 text-[12px] font-bold uppercase tracking-wide text-slate2">
            {GROUP_LABELS[category]}
          </div>
          <div
            className={`grid grid-cols-4 gap-2 sm:grid-cols-6 ${wide ? 'lg:grid-cols-8 xl:grid-cols-10' : ''}`}
          >
