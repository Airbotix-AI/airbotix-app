...
async function mockBackendAsKid(page: Page, opts: { age?: number; pro?: boolean } = {}) {
  ...
  await page.route('**/projects/*/code/turn', (route) => {
    ...
    // Debit on the turn the SECOND we charge: Lite applies the staged result (no
    // approve round-trip), Pro debits on approve — but the wallet refetch after
    // apply reads this reduced balance either way.
    stars -= 2;
    const body = opts.pro
      ? turnResult({
  ...
  // The seeded VFS read (studio opens on these files).
...
test('J2 Lite: prompt → agency beat → Do it → streamed message → files apply → game runs', async ({ page }) => {
  ...
  // "Do it" → the agent message streams (live → final) and the turn applies.
